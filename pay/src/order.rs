    ...
    if let Some(map) = params.as_object() {
        for key in map.keys().sorted() {
            vs.push(format!(
                "{}={}",
                key,
                map[key].to_string().trim_matches(&['"', '"'] as &[_])
            ));
        }
...
impl Order {
    ...
    pub async fn create(&self, mut params: serde_json::Value) -> WechatResult<serde_json::Value> {
        ...
        let body = format!(
            "<xml>{}</xml>",
            serde_xml_rs::to_string(&params).unwrap_or_default()
        );
        ...
        if let Some(root_doms) = ps.as_object() {
            let rr: serde_json::Map<String, serde_json::Value> = root_doms
                .iter()
                .map(|(i, vo)| (i.clone(), vo["$value"].clone()))
                .collect();
            ...
            if trade_type == Some("APP") && prepayid.is_some() {
                ...
                new_params.insert(
                    format!("sign"),
                    json!(gen_sign(&json!(new_params), &secret_key)),
                );
                ...
                new_params.insert(
                    format!("package"),
                    json!(format!("prepay_id={}", prepay_id)),
                );
    ...
    pub async fn refund(&self, mut params: serde_json::Value) -> WechatResult<serde_json::Value> {
        ...
        let request = Client::new_ssl(&conf.private_key, &conf.certificate);
