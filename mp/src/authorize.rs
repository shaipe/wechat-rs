...
impl WechatAuthorize {
    ...
    pub async fn create_open(&self, access_token: &str) -> WechatResult<String> {
        let uri = format!(
            "{}{}",
            API_DOMAIN,
            format!("/cgi-bin/open/create?access_token={}", access_token)
        );
        log!("uri::: {}", uri);
    ...
    pub async fn bind_open(&self, access_token: &str, open_app_id: &str) -> WechatResult<bool> {
        let uri = format!(
            "{}{}",
            API_DOMAIN,
            format!("/cgi-bin/open/bind?access_token={}", access_token)
        );
        log!("uri::: {}", uri);
    ...
    pub async fn unbind_open(&self, access_token: &str, open_app_id: &str) -> WechatResult<bool> {
        let uri = format!(
            "{}{}",
            API_DOMAIN,
            format!("/cgi-bin/open/unbind?access_token={}", access_token)
        );
        log!("uri::: {}", uri);
